...
>
    <div class="chatPickerTop">
        ...
        <Textbox bgc="black" width="100%" icon="Search" placeholder={$_("ctxMenus.searchChats")}></Textbox>
    </div>
        ...
        {#each $user?.chats || [] as child} 
            <ChatEntry click={() => {
                if ($elements.find(e => e.id.id == child.id.id && e.id.server == child.id.server)) {
                    elements.set($elements.filter(e => e.id.id + e.id.server != child.id.id + child.id.server));
                } else {
                    elements.set([
                        ...$elements, {
                            _id: child._id,
